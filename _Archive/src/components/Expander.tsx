...
const Expander = ({ children, className, maxHeight, buttonText }: any) => {
    ...
    const evalHeight = (elem: HTMLDivElement) => {
        if (!elem) return
    ...
    return (
        <div className="expander" data-show-button={showExpandButton} data-expanded={expanded}>
            <div className={expandClasses} ref={(elem: HTMLDivElement) => { evalHeight(elem) }} style={styles.expandStyle}>
                {children}
